...
const registerUser = asyncHandler(async (req, res) => {
    ...
    const user = await User.create({ name, email, password }); // creates the data in MongoDB
    if (user) {
...
const updateUserProfile = asyncHandler(async (req, res) => {
    ...
    if (user) {
        ...
        res.json({ _id: updatedUser._id, name: updatedUser.name, email: updatedUser.email, isAdmin: updatedUser.isAdmin, token: generateToken(user._id) });
    } else {
        res.status(404);
        throw new Error('User not found');
    }
