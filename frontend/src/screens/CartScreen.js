...
const CartScreen = ({ match, location, history }) => {
    ...
    return (
        <Row>
            <Col md={8}>
                <h1 style={{ fontSize: '1.4rem', padding: '1rem 0' }}>Shopping Cart</h1>
                {cartItems.length === 0 ? 
                        ...
                        {cartItems.map(item => (
                            <ListGroup.Item key={item.product}>
                                <Row>
                                    ...
                                    <Col md={2}> 
                                        <Form.Control as='select' value={item.qty} onChange={(e) => dispatch(addToCart(item.product, Number(e.target.value)))} style={{ cursor: 'pointer' }}>
